...
void RelayServer::runWriter(ThreadPool<MsgWriter>::Thread &thr) {
    ...
    while(1) {
        ...
        // Do write
        std::vector<uint64_t> deletedLevIds;

        try {
            auto t0 = std::chrono::steady_clock::now();
            auto txn = env.txn_rw();
            writeEvents(txn, neFilterCache, newEvents, 1, &deletedLevIds);
            txn.commit();
        ...
        if (searchProvider) {
            flat_hash_set<uint64_t> deletedSet(deletedLevIds.begin(), deletedLevIds.end());

            for (auto &newEvent : newEvents) {
                if (newEvent.status == EventWriteStatus::Written && !deletedSet.contains(newEvent.levId)) {
                    PackedEventView packed(newEvent.packedStr);
                    try {
